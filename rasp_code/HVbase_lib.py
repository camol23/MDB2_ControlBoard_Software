...
class HV513():
    def __init__(self, num_chips = 1, sending_time_LE = 0.001):
        ...
        # HV513 GPIO Pins
        self.BL = 19                 # Blank val         (0: Set Low HV - [~BL NAND Latch_Output] )
        self.POL = 26                # Polarity val      (1: exact in and out val - [~POL XOR Latch_Output])
        self.HI_Z = 20               # High Impedance    (0: Set high Z in the OUput)
                    # BMC GPIO
        self.CLK = 5                # Clock signal      (rise-up to shift data input)
        self.LE = 6                 # Latch enable      (1: Load D_in to D_out)
        self.DATA = 13               # Data to be sended to D_in (1-bit serial Output)
